...
from info import ADMINS, LOG_CHANNEL, SUPPORT_CHAT, MELCOW_NEW_USERS, PICS
            ...
            for u in message.new_chat_members:
                if (temp.MELCOW).get('welcome') is not None:
                    try:
                        ...
                        pass
                temp.MELCOW['welcome'] = await message.reply_photo(photo=random.choice(PICS), caption=f"👋 Hello {u.mention},\n\nWelcome To <b>'{message.chat.title}'</b> Group.")
...
async def leave_a_chat(bot, message):
    ...
    try:
        ...
        await bot.send_message(
            chat_id=chat,
            text='Hello Friends,\nMy owner has told me to leave from group so i go! If you need add me again contact my support chat group.',
            reply_markup=reply_markup,
        )
...
@Client.on_message(filters.command('bot_status') & filters.incoming)
async def get_bot_status(bot, message):
    rju = await message.reply('Getting Bot Status...')
    ...
    size = get_size(size)
    free = get_size(free)
    await rju.edit(script.BOTSTATUS_TXT.format(files, total_users, totl_chats, size, free))
