...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer(f"Hello {query.from_user.first_name},\nSend New Request!", show_alert=True)
        return
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton("⚡️ Check Bot Inbox ⚡️", url=f"https://t.me/{temp.U_NAME}")]
        )
        btn.append(
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            await bot.send_message(LOG_CHANNEL, script.NO_RESULT.format(query.message.chat.title, query.message.chat.id, query.from_user.mention, movie))
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
            ...
            await query.answer("You need to be group owner to do that!", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer(f"Hello {query.from_user.first_name},\nCheck You'r Inbox!", show_alert=True)
        except UserIsBlocked:
            await query.answer(f'Hello {query.from_user.first_name},\nYou blocked me, Unblock me and try again...', show_alert=True)
        ...
        buttons = [[
            InlineKeyboardButton("➕ Add Me To You'r Group ➕", url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ], [
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply_photo(photo=random.choice(PICS), caption=f"👋 Hello {msg.from_user.mention},\n\nI can't find the <b>'{search}'</b> you requested, Check if your spellings are correct. 😉")
        await asyncio.sleep(60)
        await k.delete()
        return
